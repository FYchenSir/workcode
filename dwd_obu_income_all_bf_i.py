...
def dwd_obu_income_fin_bf_init():
    sql1 = '''
        select 
            date
            ,scene
        ...
        from(
            select 
                date
                ,case
                    when scene = '前装签约' then '新办签约'
                    when scene = '货车个人收入新办' then '新办付费'
                    when scene = '签约' then '新办签约'
                    when scene = '货车个人收入售后' then '售后付费' 
                    when scene = '货车个人收入注销' then '注销付费' 
                end as scene
                ,case 
                    when scene = '前装签约' then '建行-首次消费时间'
                    when scene = '货车个人收入新办' then '个人付费'
                    when scene = '签约' and buyer = '通汇产品' then '通汇产品'
                    when scene = '签约' and buyer = '货车农商行' then '农商行'
                    when scene = '签约' and buyer = '货车建行' then '建行-首次消费时间'
                    when scene = '货车个人收入售后' then '个人付费' 
                    when scene = '货车个人收入注销' then '个人付费' 
                end as buyer
                ,price
                ,qianzhuang_mark
                ,obu_id
                ...
                ,'货车' as kehuo 
                ,case 
                    when scene = '前装签约' then '建行首次消费'
                    when scene = '货车个人收入新办' then '新办收入'
                    when scene = '签约' and buyer = '通汇产品' then '通汇'
                    when scene = '签约' and buyer = '货车农商行' then '激活签约'
                    when scene = '签约' and buyer = '货车建行' then '建行首次消费'
                    when scene = '货车个人收入售后' then '售后收入' 
                    when scene = '货车个人收入注销' then '注销收入'
                end as scene2
                
            
            from 
                dwd.dwd_data_tech_obu_income_yecai_mid 
            where 
                substr(date,1,7) >= '2023-01'
                and substr(date,1,7) <= '2023-06'
                and scene not like '%成本%'
                and qianzhuang_mark <> ''
            union all 
            ...
            select 
                date
                ,case 
                    when scene = '新办付费' then '新办付费'
                    when scene = '新办签约' then '新办签约'
                    when scene = '前装' and obuid in ('3701084888114727','3701084888138580') then '新办付费'
                    when scene = '前装' and obuid = '3701084888139500' then '新办签约'
                    when scene = '售后付费' then '售后付费'
                    when scene = '注销收费' then '注销收费'
                    
                end as scene
                ,case 
                    when scene = '新办付费' then '个人付费'
                    when scene = '新办签约' and kh = '货车建行' then '建行'
                    when scene = '前装' and obuid in ('3701084888114727','3701084888138580') then '个人付费'
                    when scene = '前装' and obuid = '3701084888139500' then '建行'
                    when scene = '新办签约' and kh = '货车农商行' then '农商行'
                    when scene = '售后付费' then '个人付费'
                    when scene = '注销收费' then '个人付费'
                end as buyer 
                ,price
                ,qianzhuang_mark as qianzhuang_mark2
                ,obuid
                ,'调整数据' as data_type 
                ,case 
                ...
                ,'货车' as kehuo 
                ,case 
                    when scene = '新办付费' and price < 0 then '新办个人调减'
                    when scene = '新办付费' and price >= 0 then '新办个人调增'
                    when scene = '前装' and obuid in ('3701084888114727','3701084888138580') then '新办个人调减'
                    when scene = '前装' and obuid = '3701084888139500' then '签约调减'
                    when scene = '新办签约' and kh = '货车建行' then '签约调减'
                    when scene = '新办签约' and kh = '货车农商行' and price < 0 then '签约调减'
                    when scene = '新办签约' and kh = '货车农商行' and price >= 0 then '签约调增'
                    when scene = '售后付费' and price < 0 then '售后个人调减'
                    when scene = '售后付费' and price >= 0 then '售后个人调增'
                    when scene = '注销收费' and price < 0 then '注销个人调减'
                    when scene = '注销收费' and price >= 0 then '注销个人调增'
                end as scene2
            from 
                bigdata.cdx_shenji_tiaojian_2 a 
            where
                case 
                    when obuid = '3701082224452844' then 0
                    when kh = '前装客车建行' then 1
                    when department2 = '客车' and qianzhuang_mark = '前装' then 0 
                    when kh = '货车建行' and price > 0 then 0
                    else 1 
                end = 1
                and department2 <> '客车'
                and substr(date,1,7) >= '2023-01'
                and substr(date,1,7) <= '2023-06'

            union all --1-6月减免

            select 
                obu_active_time as date
                ,'减免数据' as scene 
                ,newold as buyer
                ,0 as price
                ,qianzhuang_mark
                ...
                ,'减免数据' as scene2
            from 
                bigdata.cdx_dwd_data_tech_income_free_i_bak_0829
            where 
                substr(obu_active_time,1,7) >= '2023-01'
                and substr(obu_active_time,1,7) <= '2023-06'
            ...
            from(
                select 
                    ...
                    ,80 as price
                    ,case 
                        when  b.obuid is not null then '前装'
                        else '非前装'
                    end as qianzhuang_mark
                    ,obu_id
                from 
                    bigdata.cdx_dwd_data_tech_sign_income_ccb_sdrc_i_bak_0829 a --新办签约建行农商行
                left join(
                    select 
                        obuid
                    from
                        bigdata.cdx_data_tech_qianzhuang
                    group by 
                        1
                )b
                on a.obu_id = b.obuid
                where 
                    department = '数据科技事业部' --归属事业部
                    and channel_signed_bank = '建行'
            )tmp1 
            where 
            ...
            select 
                ...
                ,scene2
            from 
                bigdata.cdx_obu_income_all_bf_i_237_9
            where 
                kehuo = '货车'
            ...
            select  --补工行7-9月
                ...
                ,buyer as buyer
                ,180 as price
                ,qianzhuang_mark as qianzhuang_mark
                ,obu_id
                ,'应收数据' as data_type 
                ,'应收数据' as adjust_type
                ,'货车' as kehuo 
            ...
            from(
                ...
                where 
                    substr(ds,7,2) = '01'
                    and substr(a.obu_active_time,1,7) = '2023-09'
                    and ds = '20231001'
                    and channel_signed_bank like '%工%'
                group by 
                    1,2,3,4,5,6
            )
            union all 
            ...
            where 
                ...
                and substr(gmt_refund,1,7) >= '2023-01'
                and substr(gmt_refund,1,7) <= '2023-09'
            ...
            from (
                ...
                where 
                    ...
                    and department = '数据科技事业部' --归属事业部
                    and sale_department = '数据科技事业部' --销售部门
                    and substr(to_date(gmt_payment),1,7) >= '2023-01'
                    and substr(to_date(gmt_payment),1,7) <= '2023-06'
            )tmp1 
            union all 
            select 
                obu_active_time as date
            ...
            where 
                ds = '20231101'
                and substr(obu_active_time,1,7) >= '2023-01'
                and substr(obu_active_time,1,7) <= '2023-09'
                and department = '数据科技事业部'
        )t1 
        union all
        select 
            to_date('2023-08-01') as date
            ,scene
            ,buyer
            ,-180 as price
            ,qianzhuang_mark
    ...
    #spark.sql(sql1).write.partitionBy('month').saveAsTable('dwd.dwd_obu_income_fin_bf_i',mode = 'overwrite')
